...
class Particle {
    constructor(x, y) {
        ...
        const colors = ['#FFFFFF', '#FFD700', '#FFFACD', '#FFEA00'];
...
window.addEventListener('mousedown', (e) => {
    launchFireworkSequence(e.clientX, e.clientY);
});
...
function animate() {
    ...
    if (Math.random() < 0.008) {
        backgroundShootingStars.push(new ShootingStarBg());
    }

    for (let i = backgroundShootingStars.length - 1; i >= 0; i--) {
        backgroundShootingStars[i].update();
        backgroundShootingStars[i].draw();
        
        if (backgroundShootingStars[i].opacity <= 0 || backgroundShootingStars[i].y > h + 100) {
            backgroundShootingStars.splice(i, 1);
        }
    }
